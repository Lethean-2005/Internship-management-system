...
interface SlideFormProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (data: { title: string; description?: string | null; presentation_date?: string | null; file?: File | null }) => void;
  loading?: boolean;
}
...
export function SlideForm({ open, onClose, onSubmit, loading }: SlideFormProps) {
  ...
  const [title, setTitle] = useState(position ? `${position} at ${company}` : '');
        ...
        <div>
          ...
          >
            ...
            <input
              ...
              type="file"
              className="hidden"
              accept=".pdf,.pptx,.ppt,.key,.odp,.jpg,.jpeg,.png,.mp4,.zip"
